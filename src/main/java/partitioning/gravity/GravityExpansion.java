 ...
 */
public class GravityExpansion implements PartitioningStrategy {

    private final static Logger logger = LogManager.getLogger(GravityExpansion.class);
    ...
    public GravityExpansion(int numberOfIterations, int reExpansionRate, int gridSize, boolean weightedCentroid) {
    ...
    private List<Partition> bisection(Hypergraph graph) {
        ...
        for (int iteration = 0; iteration < iterations; iteration++) {
            ...
            logger.info("iteration " + (iteration + 1) + " done.");
        }
        ...
        calculateVertexDistances(graph, gravityHoles);
        for (int i = 0; i < graph.getOrder(); i++) {
            ...
            // find closest vertex
            Optional<Vertex> nearestVertex = getClosestVertex(graph, currentId);
            ...
            if (i % reExpansionRate == (reExpansionRate - 1)) {
                graph.getVertexStream().parallel().filter(Vertex::isActive).forEach(v -> this.vertexFunction(graph, v));
                calculateVertexDistances(graph, gravityHoles);
            }


        }
        logger.info("reexpansions done.");
        return partitions;
    }
    ...
    /**
     * finds the closest, active vertex to the given gravity hole index
     * The distances are precalculated with the 'calcuateVertexDistances' method.
     *
     * @param graph       the whole graph
     * @param gravityHole gravity hole index the distance shall be used
     * @return closest vertex
     */
    private Optional<Vertex> getClosestVertex(Hypergraph graph, int gravityHole) {
        return graph.getVertexStream().parallel().filter(Vertex::isActive).reduce((v1, v2) -> {
            float d1 = ((float[]) v1.getNextVertexData())[gravityHole];
            float d2 = ((float[]) v2.getNextVertexData())[gravityHole];
    ...
    /**
     * calculates the distances between each vertex and the gravity holes and stores them in the nextVertexData field
     *
     * @param graph        input graph
     * @param gravityHoles coordinates of the gravity holes
     */
    private void calculateVertexDistances(Hypergraph graph, GravityObject[] gravityHoles) {
        graph.getVertexStream().parallel().filter(Vertex::isActive).forEach(v -> {
            float[] vertexPosition = ((GravityObject) v.getVertexData()).position;
            float distance1 = VectorOps.absoluteValueOfVector(VectorOps.subtractVectors(vertexPosition, gravityHoles[0].position));
            float distance2 = VectorOps.absoluteValueOfVector(VectorOps.subtractVectors(vertexPosition, gravityHoles[1].position));

            v.setNextVertexData(new float[]{distance1, distance2});
        });
    }
     ...
     */
    private static class DensityObject {
        private int x;
        private int y;
        private float density;
